...
const actions = {
  ...
  //保存喜欢歌曲的列表
  saveFavotiteList ({ commit,state },song) {
    let favoriteList = state.favoriteList.slice() //使用slice截取的东西一定是个数组，加上这个 保证拿到的东西一定是一个数组
    favoriteList = [...favoriteList,song]
    commit(types.SAVE_PLAY_HISTORY,favoriteList)
  },
  //删除喜欢歌曲的列表
  deleteFavoriteList () {}
}
