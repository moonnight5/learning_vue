  ...
  <div class="play" v-show="playList.length>0">
    ...
    <transition name="normal" @enter="enter" @after-enter="afterEnter" @leave="leave" @after-leave="afterLeave">
      <div class="normal-player" v-show="fullScreen">
        ...
        <audio src="" ref="audio" @playing="ready" @error="error" @timeupdate="updateTime" @ended="end" @pause="paused"></audio>
      </div>
    </transition>
    <!-- 底部的播放栏 -->
...
const transionDuration = prefixStyle('transionDuration')
export default {
  name: 'play',
  mixins: [playerMixin],
  data () {
  ...
  methods: {
    ...
    changeMode () {},
    open () {
      // this.$store.dispatch('selectPlayingSong',true)
    ...
    afterLeave () {},
    back () {
      // this.$store.dispatch('selectPlayingSong',false)
      this.backFullScreen(false)
    },
    middleTouchStart () {},
    middleTouchMove () {},
    middleTouchEnd () {},
    ready () {
      ...
      //监听playing这个事件，确保慢网速或者快速切换歌曲导致的 DOM exception
      this.songReady = true
      this.canLyricPlay = true
      this.duration = this.$refs.audio.duration
      this.savePlayHistory(this.currentSong)
      //如果歌曲的播放与歌词的出现不同步，播放的时候需要同步歌词
